...
function ContextMenuInner({ bookmark, x, y, onClose }: ContextMenuProps) {
  ...
  // Compute submenu position whenever menu position or submenu visibility changes
  useEffect(() => {
    if (!showMoveSubmenu || !menuRef.current) return;
    ...
    const estimatedSubmenuHeight = Math.min(
      groups.filter((g) => g.id !== bookmark.groupId).length * 40 + 40,
      submenuMaxHeight
    );
    ...
    // Flip left if submenu would overflow right edge
    if (sx + submenuWidth > vw - 8) {
      sx = rect.left - submenuWidth - 4;
    }

    // Keep within vertical bounds
    if (sy + estimatedSubmenuHeight > vh - 8) {
      sy = vh - estimatedSubmenuHeight - 8;
    }
    if (sy < 8) sy = 8;

    setSubmenuPos({ x: sx, y: sy });
  }, [showMoveSubmenu, menuPos.x, menuPos.y, groups, bookmark.groupId]);
  ...
  return (
    <>
      ...
      {/* Submenu — sibling, not nested, so it escapes the parent overflow */}
      {showMoveSubmenu && (
        <div
          ...
          <div className="context-submenu-list">
            {moveableGroups.map((group) => (
              <button
                ...
                className="context-menu-item flex items-center gap-2 w-full px-3 py-2 text-sm text-gray-700"
              >
                <span className="w-[20px] flex items-center justify-center flex-shrink-0 text-gray-400">
                  {getGroupIcon(group.icon)}
                </span>
                <span className="truncate">{getGroupDisplayName(group, lang)}</span>
              </button>
            ))}
          </div>
        </div>
